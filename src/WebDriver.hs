...
withWebDriver logger body = do
    body $ Handle {request = webRequest logger, download = webDownload logger}

webRequest :: (FromJSON b) => Logger.Handle -> Address -> [Param] -> IO b
webRequest logger address params = do
    Logger.debug logger $ "WebDriver: Send request to " <> address
    forM_ params $ \p -> do
        case p of
            ParamText name text -> do
                Logger.debug logger $
                    "WebDriver: \t" <>
                    name <> " = \"" <> escapeText text <> "\""
            ParamTextLazy name text -> do
                Logger.debug logger $
                    "WebDriver: \t" <>
                    name <>
                    " = \"" <> TextLazy.toStrict (escapeTextLazy text) <> "\""
            ...
            ParamFile name filename payload -> do
                Logger.debug logger $
                    ...
                    " = " <>
                    filename <>
                    " (" <> Text.pack (show $ BSL.length payload) <> " bytes)"
    let hasParamFile =
            flip any params $ \p -> do
                case p of
            ...
            then do
                ...
                runReq defaultHttpConfig $
                    req
                        POST
                        url
                        (ReqBodyUrlEnc $ foldMap urlencoded params)
                        jsonResponse
                        options
    let value = responseBody resp
    Logger.debug logger $ "WebDriver: Response received"
    Logger.debug logger $ "WebDriver: \t" <> encodeToText value
    case fromJSON value of
        Success x -> do
            Logger.debug logger $ "WebDriver: Response successfully parsed"
            return $ x
        Error e -> do
            Logger.err logger $
  ...
  where
    ...
    multipart (ParamText name text) =
        Multipart.partBS name $ Encoding.encodeUtf8 text
    multipart (ParamTextLazy name text) =
        Multipart.partLBS name $ EncodingLazy.encodeUtf8 text
    multipart (ParamNum name number) =
        Multipart.partLBS name $ BSLC.pack $ show number
    multipart (ParamJson name value) = Multipart.partLBS name $ encode value
    multipart (ParamFile name filename payload) =
        Multipart.partFileRequestBody name (Text.unpack filename) $
        Client.RequestBodyLBS payload
    escapeText text = Text.replace "\"" "\\\"" $ Text.replace "\\" "\\\\" $ text
    escapeTextLazy text =
        TextLazy.replace "\"" "\\\"" $ TextLazy.replace "\\" "\\\\" $ text

webDownload :: Logger.Handle -> Address -> IO BSL.ByteString
webDownload logger address = do
    Logger.debug logger $ "WebDriver: Download a file from " <> address
    (url, options) <- parseUrlVerbose logger address
    resp <- runReq defaultHttpConfig $ req GET url NoReqBody lbsResponse options
    Logger.debug logger $ "WebDriver: Response received"

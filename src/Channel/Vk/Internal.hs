...
vkcPoll vkc = do
            ...
            case fromJSON update of
                Error err -> do
                    Logger.warn (vkcLogger vkc) $
                        "VkChannel: Failed to parse an update: " <>
                        Text.pack err
                    return $ []
  ...
  where
    ...
    requestServer = do
        Logger.info (vkcLogger vkc) $ "VkChannel: Request a long poll server"
        resp <-
            WebDriver.request
                (vkcDriver vkc)
    ...
    requestPoll (VkPollServer pollAddress pollKey pollTs) = do
        ...
        Logger.info (vkcLogger vkc) $ "VkChannel: Poll..."
        resp <-
            WebDriver.request (vkcDriver vkc) pollAddress $
        ...
        case resp of
            VkPollFail -> do
                Logger.info (vkcLogger vkc) $ "VkChannel: Poll request failed"
                return $ Nothing
            VkPollUpdates newTs updates -> do
                Logger.info (vkcLogger vkc) $ "VkChannel: Response received"
                Logger.debug (vkcLogger vkc) $
                    "VkChannel: New update offset: " <> Text.pack (show newTs)
                writeIORef (vkcPollServer vkc) $!
                    Just $! VkPollServer pollAddress pollKey newTs
                return $ Just $ updates
    failLongPoll :: IO a
    failLongPoll = do
        Logger.err (vkcLogger vkc) $
...
vkcSendMessage vkc chatId content buttons = do
    Logger.info (vkcLogger vkc) $ "VkChannel: Send message"
    Logger.debug (vkcLogger vkc) $
        "VkChannel: \t" <>
        Text.pack (show chatId) <> " <- " <> Text.pack (show content)
    Logger.debug (vkcLogger vkc) $ "VkChannel: \t" <> Text.pack (show buttons)
        ...
        then return $ Right 0
            ...
            case resp of
                ...
                VkResponse messageId -> do
                    Logger.info (vkcLogger vkc) $ "VkChannel: Message sent"
...
vkcSendMedia vkc chatId caption0 mediaList = do
    Logger.info (vkcLogger vkc) $ "VkChannel: Send media"
    Logger.debug (vkcLogger vkc) $
        "VkChannel: \t" <>
        Text.pack (show chatId) <> " <- " <> Text.pack (show caption0)
    Logger.debug (vkcLogger vkc) $ "VkChannel: \t" <> Text.pack (show mediaList)
  ...
  where
    ...
    doTrivial current caption subgroup rest = do
        ...
        resp <-
            ...
            , WebDriver.ParamText "attachment" $
              Text.intercalate "," $
              map (\(Channel.SendableMedia _ mediaId) -> mediaId) $ subgroup
            ]
        case resp of
            VkError e -> do
                Logger.err (vkcLogger vkc) $
...
vkcPossessMedia vkc chatId (Channel.ForeignMedia mediaType mediaId mediaUrl) = do
    case mediaType of
        ...
        _
            | not (Text.null mediaId || Text.isPrefixOf "!" mediaId) ->
                return $
            ...
            | otherwise -> do
                ...
                case r of
                    Left e -> do
                        Logger.err (vkcLogger vkc) $
                            "VkChannel: Failed to possess media: " <>
                            Text.pack (show (e :: SomeException))
                        return $ Channel.PossessMediaInternalError
                    Right result ->
                        return $ Channel.PossessMediaSuccess $! result
...
vkcUpdateMessage vkc chatId messageId content buttons = do
    Logger.info (vkcLogger vkc) $ "VkChannel: Update message"
    Logger.debug (vkcLogger vkc) $
        "VkChannel: \t" <>
        ...
        if messageId <= 0
            then do
                ...
                WebDriver.request
                    ...
                    "https://api.vk.com/method/messages.send" $
                    [ WebDriver.ParamText "v" $ apiVersion
                    , WebDriver.ParamText "access_token" $
                      cToken $ vkcConfig vkc
                    , WebDriver.ParamNum "peer_id" $ chatId
                    , WebDriver.ParamNum "random_id" $ randomId
                    , WebDriver.ParamTextLazy "message" $
                      Builder.toLazyText $ encodeRichTextToBuilder content
                    ] <>
                    keyboardMarkupOpt (cKeyboardWidth (vkcConfig vkc)) buttons
            else do
                WebDriver.request
    ...
    case resp of
        VkError e -> do
            ...
            return $ Left e
        VkResponse (VkVoid _) -> do
            Logger.info (vkcLogger vkc) $ "VkChannel: Message updated"
            return $ Right ()

vkcAnswerQuery ::
       VkChannel -> Channel.QueryId -> Text.Text -> IO (Either Text.Text ())
...
vkcReuploadMediaGeneral vkc chatId mediaUrl filename uploadMethodUrl uploadParams saveMethodUrl saveParamsFunc decodeMediaFunc = do
    ...
    Logger.debug (vkcLogger vkc) $ "VkChannel: Request an upload server"
    resp1 <-
        WebDriver.request (vkcDriver vkc) uploadMethodUrl $
        [ WebDriver.ParamText "v" $ apiVersion
        , WebDriver.ParamText "access_token" $ cToken $ vkcConfig vkc
        , WebDriver.ParamNum "peer_id" $ chatId
        ] <>
    ...
    uploadAddress <-
        case resp1 of
            ...
            VkResponse (VkUploadAddress address) -> do
                Logger.debug (vkcLogger vkc) $ "VkChannel: Got an upload server"
                return $ address
    Logger.debug (vkcLogger vkc) $ "VkChannel: Download the media"
    mediaContent <- WebDriver.download (vkcDriver vkc) mediaUrl
    Logger.debug (vkcLogger vkc) $ "VkChannel: Upload the media"
    uploadResult <-
        WebDriver.request (vkcDriver vkc) uploadAddress $
        [WebDriver.ParamFile "file" filename mediaContent]
    Logger.debug (vkcLogger vkc) $ "VkChannel: Save the media"
    resp2 <-
        WebDriver.request (vkcDriver vkc) saveMethodUrl $
        [ WebDriver.ParamText "v" $ apiVersion
        , WebDriver.ParamText "access_token" $ cToken $ vkcConfig vkc
        ] <>
...
encodeRichTextToBuilder (Channel.RichTextLink href title next) = do
    encodeRichTextToBuilder title <>
        " (" <> Builder.fromText href <> ")" <> encodeRichTextToBuilder next
encodeRichTextToBuilder (Channel.RichTextMention user title next) = do
    encodeRichTextToBuilder title <>
        " (https://vk.com/id" <>
        Builder.fromText user <> ")" <> encodeRichTextToBuilder next
...
instance FromJSON VkPollResponse where
    parseJSON =
        withObject "VkPollResponse" $ \v -> do
            msum
...
instance FromJSON VkUpdate where
    parseJSON =
        withObject "VkUpdate" $ \v -> do
            typename <- v .: "type"
            case typename of
                "message_new" ->
                    v .: "object" >>= (.: "message") >>= parseMessage
                _ -> return $ VkUpdateUnknown $! typename
      where
        parseMessage =
            withObject "VkUpdateMessage" $ \v -> do
                text <- v .:? "text" .!= ""
                chatId <- v .: "peer_id"
                messageId <- v .: "conversation_message_id"
                msum
                    [ do payload <- v .: "payload"
                         Just payloadInner <-
                             return $
                             Text.stripSuffix "\"" =<<
                             Text.stripPrefix "\"" payload
                         return $
                             VkUpdateEvent $!
                             Channel.EventQuery
                                 ...
                                 , Channel.eUserdata =
                                       Text.replace "\\\\" "\\" $
                                       Text.replace "\\\"" "\"" $ payloadInner
                                 }
                    , do attachments <- v .: "attachments"
                             ...
                             Channel.EventMedia
                                 ...
                                 , Channel.eMedia =
                                       if chatId >= 2000000000
                                           then map getPossessableForeignMedia
                                                    attachments
                                           else map getResendableForeignMedia
                                                    attachments
                                 }
                    , do return $
                             VkUpdateEvent $!
...
instance FromJSON VkForeignMedia where
    parseJSON =
        withObject "VkForeignMedia" $ \v -> do
            typename <- v .: "type"
            case typename :: Text.Text of
                ...
                _ ->
                    return $
                    VkForeignMedia $!
                    Channel.ForeignMedia Channel.MediaUnknown typename ""
      where
        parsePhoto =
            withObject "VkForeignMediaPhoto" $ \v -> do
                mediaId <- getMediaId "photo" v
                sizes <- v .:? "sizes" .!= []
                if null (sizes :: [VkPhotoSize])
                    then return $
                         VkForeignMedia $!
                         Channel.ForeignMedia Channel.MediaPhoto mediaId ""
                    else do
                        let VkPhotoSize _ _ url =
                                flip maximumBy sizes $ \(VkPhotoSize w1 h1 _) (VkPhotoSize w2 h2 _) -> do
                                    compare w1 w2 <> compare h1 h2
                        return $
                            VkForeignMedia $!
                            Channel.ForeignMedia Channel.MediaPhoto mediaId url
        parseVideo =
            withObject "VkForeignMediaPhoto" $ \v -> do
                mediaId <- getMediaId "video" v
                return $
                    VkForeignMedia $!
                    Channel.ForeignMedia Channel.MediaVideo mediaId ""
        parseAudio =
            withObject "VkForeignMediaAudio" $ \v -> do
                mediaId <- getMediaId "audio" v
                url <- v .:? "url" .!= ""
                return $
                    VkForeignMedia $!
                    Channel.ForeignMedia Channel.MediaAudio mediaId url
        parseDoc =
            withObject "VkForeignMediaDocument" $ \v -> do
                ...
                return $
                    ...
                    Channel.ForeignMedia
                        ...
                        mediaId
                        (Text.pack (show filename) <> url)
        parseSticker =
            withObject "VkForeignMediaSticker" $ \v -> do
                stickerId <- v .: "sticker_id"
                let stickerIdStr = show (stickerId :: Integer)
                return $
                    VkForeignMedia $!
                    Channel.ForeignMedia
                        Channel.MediaSticker
                        (Text.pack stickerIdStr)
                        ""
        parseVoice =
            withObject "VkForeignMediaVoice" $ \v -> do
                url <- v .: "link_mp3"
                return $
                    VkForeignMedia $!
                    Channel.ForeignMedia Channel.MediaVoice "" url
...
instance FromJSON VkPhotoSize where
    parseJSON =
        withObject "VkPhotoSize" $ \v -> do
            VkPhotoSize <$> v .: "width" <*> v .: "height" <*> v .: "url"
...
instance FromJSON VkUploadAddress where
    parseJSON =
        withObject "VkUploadAddress" $ \v -> do
...
instance FromJSON VkPhotoUploadResult where
    parseJSON =
        withObject "VkPhotoUploadResult" $ \v -> do
            VkPhotoUploadResult <$> v .: "server" <*> v .: "photo" <*>
                v .: "hash"
...
instance FromJSON VkSendablePhoto where
    parseJSON =
        withObject "VkSendablePhoto" $ \v -> do
            mediaId <- getMediaId "photo" v
            return $
                VkSendablePhoto $!
                Channel.SendableMedia Channel.MediaPhoto mediaId
...
instance FromJSON VkDocumentUploadResult where
    parseJSON =
        withObject "VkDocumentUploadResult" $ \v -> do
...
instance FromJSON VkSendableDocument where
    parseJSON =
        withObject "VkSendableDocument" $ \v -> do v .: "doc" >>= parseDocument
      where
        parseDocument =
            withObject "VkMediaDocument" $ \v -> do
                mediaId <- getMediaId "doc" v
                return $
                    VkSendableDocument $!
                    Channel.SendableMedia Channel.MediaDocument mediaId
...
instance FromJSON VkSendableVoice where
    parseJSON =
        withObject "VkSendableVoice" $ \v -> do
            v .: "audio_message" >>= parseVoice
      where
        parseVoice =
            withObject "VkMediaVoice" $ \v -> do
                mediaId <- getMediaId "doc" v
                return $
                    VkSendableVoice $!
                    Channel.SendableMedia Channel.MediaVoice mediaId
...
getMediaIdCommon publicOnly typename v = do
    ...
    case maccessKey of
        Nothing ->
            return $ Text.pack $ typename <> ownerIdStr <> "_" <> localIdStr
        Just accessKey ->
            if publicOnly
                then return $ ""
                else return $
                     Text.pack $
                     typename <>
                     ownerIdStr <> "_" <> localIdStr <> "_" <> accessKey

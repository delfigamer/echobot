...
withRepeatResponder conf logger channel body = do
    ...
    body $ Responder.Handle {Responder.work = erWork er}
            ...
            , 1 <= mult && mult <= cMaxMultiplier (erConfig er) -> do
                ...
                erMakeRequest er "updateMessage" $
                    Channel.updateMessage
                        ...
                        messageId
                        (Channel.plainText $
                         substitute [show mult] $
                         cMultiplierSetMsg (erConfig er))
                        []
        _ -> return ()

erRepeatMessage ::
       RepeatResponder -> Channel.ChatId -> Channel.RichText -> IO ()
erRepeatMessage er chatId content = do
    mult <- erGetMultiplier er chatId
    Logger.debug (erLogger er) $
        "Responder: repeat a message into " <>
        Text.pack (show chatId) <> " " <> Text.pack (show mult) <> " times"
    replicateM_ mult $ do
...
erRepeatMedia er chatId caption inGroup = do
    mult <- erGetMultiplier er chatId
    Logger.debug (erLogger er) $
        "Responder: repeat media into " <>
        Text.pack (show chatId) <> " " <> Text.pack (show mult) <> " times"
    possessionResult <-
        forM inGroup $ Channel.possessMedia (erChannel er) chatId
    ...
    forM_ unknownTypes $ \typename -> do
        erMakeRequest er "sendMessage" $
            Channel.sendMessage
                ...
                chatId
                (Channel.plainText $
                 substitute [Text.unpack typename] $
                 cMediaUnknownTypeMsg (erConfig er))
                []
  ...
  where
    selectSuccess [] = []
    selectSuccess (Channel.PossessMediaSuccess outMedia:rest) =
        outMedia : selectSuccess rest
    selectSuccess (_:rest) = selectSuccess rest
    selectUnknownType [] = []
    selectUnknownType (Channel.PossessMediaUnknownType typename:rest) =
        typename : selectUnknownType rest
...
erHandleCommand er chatId cmd
    ...
    | Text.isPrefixOf (cInspectMultiplierCmd (erConfig er)) cmd = do
        mult <- erGetMultiplier er chatId
        Logger.debug (erLogger er) $
            "Responder: requested multiplier in " <>
            Text.pack (show chatId) <>
            " (currently " <> Text.pack (show mult) <> ")"
        erMakeRequest er "sendMessage" $
            Channel.sendMessage
                (erChannel er)
                chatId
...
erSetMultiplier er chatId mult = do
    ...
    Logger.info (erLogger er) $
        "Responder: set multiplier in " <>
        Text.pack (show chatId) <> " to " <> Text.pack (show mult)
...
erProvideChatContext er chatId = do
    ...
    case mcc of
        Nothing -> do
            cc <- ChatContext <$> newIORef Nothing
            IOTable.insert (erContextTable er) chatId cc
            Logger.info (erLogger er) $
                "Responder: allocate a new context in " <>
                Text.pack (show chatId)
            return $ cc
        Just cc -> do
            return $ cc
...
erMakeRequest ::
       RepeatResponder -> Text.Text -> IO (Either Text.Text a) -> IO ()
  ...
  where
    ...
    walk (p:ps) = do
        case Text.uncons p of
            Just (c, rest)
                | '1' <= c && c <= '9' ->
                    arg (fromEnum c - fromEnum '1') : rest : walk ps

...
main = do
    ...
    withLogger (acLogger ac) $ \logger -> do
        let driverlogger =
                Logger.loggerFilter (logLevel $ acWebDriverLogLevel ac) logger
        WebDriver.withWebDriver driverlogger $ \driver -> do
            let channellogger =
                    Logger.loggerFilter (logLevel $ acChannelLogLevel ac) logger
            withChannel (acChannel ac) channellogger driver $ \channel -> do
                let responderlogger =
                        Logger.loggerFilter
                            (logLevel $ acResponderLogLevel ac)
                            logger
                withResponder (acResponder ac) responderlogger channel $ \responder -> do
                    catch (forever $ Responder.work responder) (logFatal logger)
  where
    logFatal :: Logger.Handle -> SomeException -> IO ()
    logFatal logger err = do
        Logger.err logger $ "Fatal error: " <> Text.pack (show err)
...
withLogger (FileLoggerConfig filepath) body =
    Logger.withFileLogger filepath body
withLogger (MultiLoggerConfig logs) body0 = do
  ...
  where
    ...
    withMulti (a:rest) body = do
        withLogger a $ \la -> do
            withMulti rest $ \lrest -> do Logger.withMultiLogger la lrest body
withLogger (FilterLoggerConfig (LogLevelConfig level) logconf) body = do
    withLogger logconf $ \la -> do body $ Logger.loggerFilter level la

withResponder ::
       ResponderConfig
    -> Logger.Handle
    -> Channel.Handle
    -> (Responder.Handle -> IO r)
    -> IO r
...
instance FromJSON AppConfig where
    parseJSON =
        withObject "AppConfig" $ \v -> do
            AppConfig <$> v .: "channel" <*> v .:? "logger" .!= StdLoggerConfig <*>
                v .: "responder" <*>
...
instance FromJSON ChannelConfig where
    parseJSON =
      ...
      where
        parseTg v = do
            (TgChannelConfig <$>) $
                Channel.Tg.Config <$> v .: "tg-token" <*> v .:? "timeout" .!= 60 <*>
                v .:? "keyboard-width" .!= 5
        parseVk v = do
            (VkChannelConfig <$>) $
                Channel.Vk.Config <$> v .: "vk-token" <*> v .: "vk-group-id" <*>
                v .:? "timeout" .!= 60 <*>
                v .:? "keyboard-width" .!= 5
      ...
      where
        ...
        asObject =
            withObject "LoggerConfig" $ \v -> do
                ...
                parser obj
        parseFile =
            withText "FileLoggerConfig" $ \path -> do
                return $ FileLoggerConfig (Text.unpack path)
        parseFilter =
            withObject "FilterLoggerConfig" $ \v -> do
...
instance FromJSON ResponderConfig where
    parseJSON =
        withObject "ResponderConfig" $ \v -> do
            (parser, obj) <- msum [(,) parseRepeat <$> v .: "repeat"]
            parser obj
      where
        parseRepeat =
            withObject "RepeatResponderConfig" $ \v -> do
                (RepeatResponderConfig <$>) $
                    Responder.Repeat.Config <$> v .:? "default-multiplier" .!= 2 <*>
                    v .:? "unknown-command-msg" .!=
                    "[responder.repeat.unknown-command-msg]" <*>
                    v .:? "start-cmd" .!= "/start" <*>
                    v .:? "start-msg" .!= "[responder.repeat.start-msg] %1" <*>
                    v .:? "describe-cmd" .!= "/help" <*>
                    v .:? "describe-msg" .!=
                    "[responder.repeat.describe-msg] %1" <*>
                    v .:? "inspect-multiplier-cmd" .!= "/repeat" <*>
                    v .:? "inspect-multiplier-msg" .!=
                    "[responder.repeat.inspect-multiplier-msg] %1" <*>
                    v .:? "multiplier-set-msg" .!=
                    "[responder.repeat.multiplier-set-msg] %1" <*>
                    v .:? "media-unknown-type-msg" .!=
                    "[responder.repeat.media-unknown-type-msg] %1" <*>
                    v .:? "media-unsupported-msg" .!=
                    "[responder.repeat.media-unsupported-msg]" <*>
                    v .:? "media-internal-error-msg" .!=
                    "[responder.repeat.media-internal-error-msg]" <*>
                    v .:? "max-multiplier" .!= 5
